...
static pthread_mutex_t s_drsMutex = PTHREAD_MUTEX_INITIALIZER;
   ...
   for (index = 0; index < 127; index++) {
      ...
      if (musb_open(&usb_interface, 0x04B4, 0x1175, index, 1, 0) == MUSB_SUCCESS) {
         ...
         if (musb_get_device(usb_interface) != 1) {
            ...
            int stuck_count = 0;
            do {
               i = musb_read(usb_interface, 8, buffer, sizeof(buffer), 1000);
               if (i > 0) {
                  if (stuck_count < 3)
                     printf("%d bytes stuck in buffer\n", i);
                  stuck_count++;
               }
            } while (i > 0 && stuck_count < 100);
            if (stuck_count > 3)
               printf("... drained %d total buffers\n", stuck_count - 3);
...
DRSBoard::~DRSBoard()
{
   ...
   pthread_mutex_destroy(&s_drsMutex);
...
int DRSBoard::Write(int type, unsigned int addr, void *data, int size)
{
   ...
   if (fTransport == TR_VME) {
      ...
      pthread_mutex_unlock(&s_drsMutex);
      return size;
#endif                          // HAVE_VME
...
int DRSBoard::ReadMagic(unsigned char *buffer)
{
   // Read magic number register (REG_MAGIC = 0x00)
   return Read(T_STATUS, buffer, REG_MAGIC, 2);
}
